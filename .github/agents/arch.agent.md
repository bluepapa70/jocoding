...
## 프로젝트 아키텍처 개요 (회식 메뉴 투표 시스템)

본 프로젝트는 **FastAPI 기반 회식 메뉴 투표 백엔드 시스템**입니다.

### 계층 구조 (Layered Architecture)
```
app/
├── api/          # 프레젠테이션 계층 — FastAPI Routers (events.py)
├── service/      # 비즈니스 로직 계층 — event/menu/vote 서비스
├── repository/   # 데이터 접근 계층 — SQLite ORM 접근
├── models/       # 도메인 계층 — SQLAlchemy ORM 모델
└── schemas/      # DTO 계층 — Pydantic 스키마
```

### 주요 컴포넌트
- **Event** — 회식 이벤트 생성/조회
- **Menu** — 이벤트별 메뉴 후보 제안/조회
- **Vote** — 이벤트별 메뉴 투표
- **Results** — 이벤트별 투표 결과 집계

### 기술 스택
- Python 3.x, FastAPI, SQLAlchemy 2.x, SQLite, Pydantic v2
