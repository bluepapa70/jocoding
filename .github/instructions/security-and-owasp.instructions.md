...
## 프로젝트 보안 컨텍스트 (회식 메뉴 투표 시스템)

- **A03 Injection**: SQLAlchemy ORM 파라미터 바인딩만 허용, raw SQL 금지
- **A05 Misconfiguration**: SQLite DB 파일 경로를 환경변수로 관리, 외부 노출 금지
- **A09 Logging**: 에러 응답 시 내부 스택트레이스 미포함 (표준 HTTP 상태코드만 반환)
- **익명 참여 구조**: 중복 투표 방지를 위한 비즈니스 로직 검증 필수
- **입력 검증**: 모든 API 요청은 Pydantic v2 스키마를 통해 검증 (type, required 필드)
