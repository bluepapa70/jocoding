...
## 프로젝트 모듈 구조 참조 (회식 메뉴 투표 시스템)

작업 분해 시 아래 모듈 계층을 기준으로 단계를 구성하세요:

1. **schemas** ? Pydantic DTO 정의 (입력/출력 모델)
2. **models** ? SQLAlchemy ORM 테이블 정의
3. **repository** ? DB CRUD 로직
4. **service** ? 비즈니스 로직
5. **api (router)** ? HTTP 엔드포인트 (FastAPI Router)
6. **test** ? pytest 기반 통합 테스트 (TDD: 테스트 먼저 작성)

TDD 원칙에 따라 항상 테스트 작성 단계를 구현 이전에 배치하세요.
